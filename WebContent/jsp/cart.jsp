	...
	<body>
		...
		<c:if test="${not (empty cart||empty cart.cartItems) }">
				...
				<div style="margin:0 auto; margin-top:10px;width:950px;">
					...
					<c:forEach items="${cart.cartItems}" var="item">
							<tr class="active">
								...
								<td width="10%">
									<!-- 
									<input type="text" name="count" value="${item.count}" maxlength="4" size="10">
			...
			<c:if test="${empty user }">
				<em style="color:#ff6600;">尚未登录,登录后确认是否享有优惠&nbsp;&nbsp;</em> 
			</c:if>
			<c:if test="${not empty user }">
			<fmt:formatNumber value="${cart.total }" var="t" pattern="#"></fmt:formatNumber>
				赠送积分: <em style="color:#ff6600;">${t }</em>
			</c:if>
