...
<HTML>
	...
	<body>
		...
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/UpdateProductServlet" method="post" enctype="multipart/form-data">
			...
			<input type="hidden" name="image" value="${p.pimage }">
			...
			<table cellSpacing="1" cellPadding="5" width="100%" align="center" bgColor="#eeeeee" style="border: 1px solid #8ba7e3" border="0">
				...
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff">
						...
						<select name="is_hot">
							<option value="1"  ${p.is_hot eq 1 ? "selected='selected'":"" }>是</option>
							<option value="0"  ${p.is_hot eq 0 ? "selected='selected'":"" }>否</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td class="ta_01" bgColor="#ffffff">
						<input type="text" name="mark_price" value="${p.mark_price }" id="userAction_save_do_logonName" class="bg"/>
					</td>
					<td width="18%" align="center" bgColor="#f5fafe" class="ta_01">
						商城价格：
					</td>
					...
					<td class="ta_01" bgColor="#ffffff" colspan="3">
						<select name="cid">
							<c:forEach items="${list }" var="c">
								<option value="${c.cid }"  ${c.cid eq p.category.cid ? "selected='selected'":"" } >${c.cname }</option>
							</c:forEach>
						</select>
					</td>
				</tr>
